...
class ExerciseViewModel(
    ...
    private lateinit var currentExercise: SessionExercise
    private lateinit var routine: Routine
    private var restAnimator: ValueAnimator
    private lateinit var sessionModel: SessionManager

    init {
        viewModelScope.launch {
            routine = dao.getRoutine(routinName)
            sessionModel = SessionManager(dao, routine)
            sessionModel.getExercises()
            setExercise()
        ...
        restAnimator = animationProgress (time = 90000 ,onValueChange = {percent: Float ->  state.update { it.copy(timerPercent = percent)}})
        restAnimator.doOnEnd { _state.update { it.copy(resting = false) } }
    }
    ...
    fun onEvent(event: ExerciseEvent){
        when(event){
            ExerciseEvent.FinishedSet -> {
                viewModelScope.launch {
                    currentExercise.updateSet(state.value.repsCompleted)
                    if (state.value.setNum == currentExercise.setNum) {
                        setExercise()
                    } else {
                        state.update {
                            it.copy(
                        ...
                        animationProgress(calulateCircularPercentage(),
            ...
            ExerciseEvent.SkipRest -> {
                restAnimator.end()
            }

            is ExerciseEvent.UpdateWeight -> {
                currentExercise.weight = event.n.toFloat()
                state.update { it.copy(weight = currentExercise.weight) }
            }
        }
    }

    private suspend fun setExercise(){
        val exercise = sessionModel.getNextExercise()
        if (exercise == null){
            ...
            state.update { it.copy(
                ...
                setNum = 1,
                repsCompleted = currentExercise.reps,
                totalNumSetsTodo = currentExercise.setNum
            ) }
        }
    }

    private fun calulateCircularPercentage(): Float {
        return ((state.value.setNum - 1).toFloat() / state.value.totalNumSetsTodo) / (state.value.setNum.toFloat() / state.value.totalNumSetsTodo)
    }

    // dont need when saved at end
...
//    }
    ...
    private fun animationProgress(start: Float = 0f, time: Long = 1000, onValueChange: (percent:Float)-> Unit): ValueAnimator {
